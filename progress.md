...
- **Garde-fou anti-double-soumission généralisé dans `PaymentServiceImp`** : `processPayment()` bloquait uniquement sur un attempt existant `SUCCESS` ; étendu à `SUCCESS || PROCESSING` (mirroring `retryPayment`'s garde-fou), et `refund()` (qui n'avait AUCUN contrôle) reçoit désormais le même. Ajout d'un filet de sécurité race-safe sur les deux méthodes : `DataIntegrityViolationException` sur l'insert (contrainte `uk_payment_idempotency`) est catchée et traduite en `PaymentAlreadyCompletedForThisOrderException` (409) au lieu d'un 500 brut — pattern calqué sur `WishlistServiceImp.addToWishlist`. Ce même catch couvre aussi le cas de deux appels concurrents à `cancelOrder`/`refund` sur la même commande (pas de lock nécessaire, la contrainte DB + le catch suffisent). 4 tests ajoutés dans `PaymentServiceImpTest` (22 tests au total, tous verts, + 344/344 sur l'ensemble des tests order/payment).
...
- Inventaire de toute la documentation du projet (voir liste dans la conversation / `docs/`, `LOCAL_LAUNCH_CHECKLIST.md`, etc.).
- Jenkins retiré du projet à la demande du propriétaire (ne sera pas utilisé) : `Jenkinsfile`, `docker-compose.jenkins.yml`, dossier `jenkins/` supprimés ; toutes les mentions dans `readme.md`, `DEPLOY_MINIKUBE.md` et `.gitignore` nettoyées (renumérotation des étapes optionnelles de `DEPLOY_MINIKUBE.md`).
- **Clé d'idempotence des paiements enrichie avec les quantités** (`placeOrder` + `updateOrder` Cas 1) : le contexte hashé était `orderUuid + UUIDs produits` (ignorait les quantités), devient `orderUuid + "UUID:quantité"` par ligne. Corrige au passage le bug `updateOrder`-Cas 2 documenté plus haut : `handlePaymentUpdate` résout désormais la clé du paiement ORIGINAL (le plus récent `PaymentAttempt` `PAYMENT`/`SUCCESS`, via son `getIdempotencyKey()`) au lieu de repasser la clé fraîchement recalculée à `refund()` — mirroring `OrderCancellationTransactionalDelegateImp`. Test de non-régression ajouté dans `OrderManagementServiceImpTest` (69 tests, tous verts).
- **Micrometer retiré entièrement du projet** (décision explicite du propriétaire, scope confirmé via question de clarification : "tout retirer, y compris l'observabilité") :
  - Code : champs `MeterRegistry` + compteurs `cybertech.payments.total`/`cybertech.orders.total` supprimés de `PaymentServiceImp`/`OrderManagementServiceImp` ; commentaire obsolète nettoyé dans `NotificationRetryableDeliveryImp`.
  - `pom.xml` : dépendances `micrometer-registry-prometheus`, `micrometer-tracing-bridge-otel`, `opentelemetry-exporter-otlp` retirées (`logstash-logback-encoder` conservé — sert Loki, indépendant de Micrometer).
  - `application.properties` : bloc observability metrics/tracing retiré (`management.endpoint.prometheus.enabled`, `management.metrics.tags.application`, `management.tracing.*`, `management.otlp.*`) ; `prometheus` retiré de `management.endpoints.web.exposure.include`.
  - `SecurityConfig.java` : entrée `permitAll` `/actuator/prometheus` retirée.
  - `logback-spring.xml` : `includeMdcKeyName` `traceId`/`spanId` retirés (n'étaient peuplés que via le tracing bridge Micrometer, jamais posés manuellement en MDC).
  - Helm : charts `prometheus-chart` et `tempo-chart` supprimés entièrement ; `grafana-chart` réduit à un datasource Loki seul (4 dashboards metrics/tracing supprimés, `values.yaml`/`datasources-configmap.yaml`/`Chart.yaml` mis à jour, toggle `traceqlEditor` retiré) ; `helmfile.yaml.gotmpl` layer 6 réduite à `loki`+`grafana` ; annotations `prometheus.io/scrape` retirées de `cybertech-app-chart`.
  - Docs : `readme.md` (diagramme d'archi, tableaux, points forts), `DEPLOY_MINIKUBE.md` (Step 7 réécrit), `k8s/helm/README.md` (section 7b réécrite), `LOCAL_LAUNCH_CHECKLIST.md` (note C.4 obsolète retirée) — tous mis à jour pour ne plus mentionner Prometheus/Tempo.
  - Build vérifié après coup : `mvn compile` + `test-compile` + suite unitaire complète → 1971 tests, 0 échec.
...
- **Résidu du fix idempotence sur `updateOrder` Cas 1 (paiement du complément)** : sa clé d'idempotence (`updateIdempotencyKey`, `OrderManagementServiceImp`) reste stable pour une composition d'items donnée (dérivée de `orderUuid + produits:quantités`), contrairement à `retryPayment` qui fait varier sa clé à chaque tentative. Conséquence : un retry légitime après un refus de carte sur cette voie précise se fait bloquer par `PaymentAlreadyCompletedForThisOrderException` (409) au lieu de planter en 500 — plus sûr, mais bloque une vraie tentative de re-paiement. Pas corrigé, nécessiterait de dupliquer le pattern attempt-counter de `retryPayment` pour la branche Cas 1 uniquement.
...
- **Aucune réconciliation active si un webhook Stripe se perd définitivement** (au-delà de la fenêtre de retry de 3 jours de Stripe — mauvaise config d'endpoint pendant un déploiement, incident Stripe, etc.) : la commande resterait bloquée indéfiniment en `AWAITING_PAYMENT`/`CREATED` avec du stock réservé, sans qu'aucun job ne vienne interroger l'API Stripe pour se resynchroniser (`GetAllFailedPaymentOrderTasklet` ne couvre que les commandes déjà passées en `PAYMENT_FAILED`, pas ce cas). Décision explicite du propriétaire : cas jugé extrêmement rare, la complexité d'un job de réconciliation active (polling Stripe) n'est pas justifiée pour ce projet. Documenté, pas d'action prévue.
