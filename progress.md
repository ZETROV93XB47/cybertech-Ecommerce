...
| BUG-161 | HIGH | CartServiceImp#deleteByUUID / BankCard analog | `deleteByUUID(UUID)` takes no caller arg — trivial IDOR. Same shape in `BankCardManagementServiceImp`. | `CartServiceImpTest@Disabled("BUG-161")` + `BankCardManagementServiceImpTest@Disabled("BUG-161")` + `CartFlowIT@Disabled("BUG-161")` | **REOPENED** (W2.2 + W3.4 + W5.2 source-refuted F1: `UnauthorizedCartAccessException` does not exist) |
