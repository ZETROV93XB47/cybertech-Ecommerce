...
public class ActiveUserValidator extends ChainableOrderValidator {
    ...
    /** Centralise the contract message so error response bodies stay consistent. */
    ...
    public void validate(final OrderValidationDto orderValidationDto) {
        ...
        if (!orderValidationDto.isUserActive()) {
            // IllegalStateException is not handled in ErrorManagementController, so the previous
            // code surfaced an opaque HTTP 500 to the client. UserNotActiveException is
