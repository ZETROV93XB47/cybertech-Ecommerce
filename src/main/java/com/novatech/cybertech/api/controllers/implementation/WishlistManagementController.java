...
public class WishlistManagementController implements WishlistManagementApiSpec {
    ...
    @PreAuthorize("isAuthenticated()")
    public ResponseEntity<Collection<WishlistResponseDto>> getMyWishlist(@AuthenticationPrincipal final Jwt jwt) {
        return ResponseEntity.ok(wishlistService.getMyWishlist(jwt.getSubject()));
    }
